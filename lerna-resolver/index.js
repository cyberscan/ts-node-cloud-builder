...
program
    ...
    .requiredOption('-a, --application <package_name>', 'application to copy packages for');
...
function cli() {
    if(options.debug) console.log("Info: options: " + JSON.stringify(options, null, 2));
    ...
    const result = execSync(`lerna ls -a -l --json --scope=${options.application} --include-dependencies`, { encoding: "utf-8", stdio: "pipe" });
    let parsedLernaLsResult = JSON.parse(result.trim());

    // resolve application dir
    const applicationPackage = parsedLernaLsResult.filter( package => package.name === options.application)[0];
    if(!applicationPackage) throw "Error: could not determine main application";

    if(options.debug) {
        console.log("Info: following application will be bootstrapped: ", JSON.stringify(applicationPackage, null, 2));
    } else {
        console.log(`Info: main application ${applicationPackage.name}`);
    }

    // resolve application dependencies (packages)
    const applicationPackageDependencies = parsedLernaLsResult.filter( package => package.name !== options.application);
    // resolve application node_modules
    const applicationNodeModulesFolder = normalize(join(applicationPackage.location, 'node_modules'));

    for(let package of applicationPackageDependencies) {
        const packageDestination = normalize(join(applicationNodeModulesFolder, package.name))
...
function processPackage(package, destinationFolder) {
    // dereference: follow symlinks and copy instead of copy symlink
    const copyOptions = {
        dereference: true
    };
